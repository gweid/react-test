...
React Hooks 是 React 16.8 引入的一种**函数式组件增强机制**，它允许函数组件使用状态（state）、生命周期（lifecycle）、上下文（context）等原本只有类组件才有的特性。



**hook 的核心原理：**

- **闭包 + 链表存储状态**。React Hooks 的实现依赖于 **闭包（Closure）** 和 **链表（Linked List）**
  - **闭包**：每个 Hook（如 `useState`、`useEffect`）在函数组件内部运行时，会形成一个闭包，保存当前的状态或副作用逻辑。
  - **链表**：React 内部维护一个 **Hook 链表**，按顺序记录每个 Hook 的调用顺序和状态值。

- **状态与组件分离**
  - **类组件**：状态（`this.state`）和生命周期方法（`componentDidMount`）绑定在组件实例上。
  - **函数组件 + Hooks**：状态和副作用逻辑通过 Hook **外挂到 Fiber 节点**（React 的虚拟 DOM 结构），与组件解耦。
...
- 解决类组件的痛点：
  - **逻辑复用困难**：类组件的复用依赖 HOC（高阶组件）或 Render Props，导致嵌套地狱。
  - **生命周期方法臃肿**：`componentDidMount`、`componentDidUpdate` 等逻辑分散。
  - **`this` 绑定问题**：类组件需要手动绑定事件处理器（如 `this.handleClick = this.handleClick.bind(this)`）。
- 函数式编程思想
  - **纯函数**：函数组件更符合 React 的“UI = f(state)”理念。
  - **副作用隔离**：通过 `useEffect` 集中管理副作用，避免污染渲染逻辑。
